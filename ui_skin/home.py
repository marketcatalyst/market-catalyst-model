...
if not st.session_state["authenticated"]:
    ...
    with auth_col2:
        business_name = st.text_input("Registered Corporate/Trading Name", placeholder="e.g., NewCo Logistics Ltd")
...
# =============================================================================
    ...
    # 🕹️ Add Databank Toggle configuration
    dataset_mode = st.radio(
        "Select Dataset Initialization Workspace Mode:",
        ["Initialize Fresh Template (Clean Slate for New Users)", "Load AHOTG Expansion Plan (Verified Reference Case Scenario)"],
        index=0,
        horizontal=True
    )
    ...
    with st.form("winforecast_setup_wizard"):
        ...
        with config_col1:
            default_proj_name = "Core Strategy Expansion Plan" if "AHOTG" in dataset_mode else "Strategic 5-Year Outlook Baseline"
            project_title = st.text_input("Forecast Project Model Name", value=default_proj_name)
        ...
        with config_col2:
            ...
            default_gross_margin = 30 if "AHOTG" in dataset_mode else 50
            gross_margin_baseline = st.slider("Target Base Production Gross Profit Coefficient (%)", min_value=10, max_value=90, value=default_gross_margin)
        ...
        # Populate defaults conditionally based on chosen mode
        val_cash = 69488.00 if "AHOTG" in dataset_mode else 0.00
        val_ar = 44886.00 if "AHOTG" in dataset_mode else 0.00
        val_fa = 531385.00 if "AHOTG" in dataset_mode else 0.00
        
        with seed_col1:
            opening_cash = st.number_input("Liquid Cash held at Bank (£)", value=val_cash, step=1000.00)
        with seed_col2:
            opening_ar = st.number_input("Outstanding Trade Debtors / Accounts Receivable (£)", value=val_ar, step=1000.00)
        with seed_col3:
            opening_fa = st.number_input("Fixed Asset Carrying Book Value (NBV) (£)", value=val_fa, step=5000.00)

        submit_setup = st.form_submit_button("🏁 Compile Relational Three-Way Database Structure", type="primary")
        
        if submit_setup:
            if "AHOTG" in dataset_mode:
                # Retain the exact verified historical tracking records for verification checks
                st.session_state["baseline_inputs"] = {
                    ...
                    "meta_project_name": project_title, "meta_horizon_years": forecast_extent_years, "meta_year_end": financial_year_end_month
                }
            else:
                # 🟢 Construct pure blank configuration vectors for a completely fresh user workspace
                st.session_state["baseline_inputs"] = {
                    "opening_cash_balance": opening_cash,
                    "opening_fixed_assets_nbv": opening_fa,
                    "opening_accounts_receivable": opening_ar,
                    "opening_accounts_payable": 0.0,
                    "opening_long_term_debt": 0.0,
                    "opening_inventory_balance": 0.0,
                    "y1_revenue_target": 1200000.00,       # Base default boilerplate benchmarks
                    "y2_revenue_target": 1500000.00,
                    "y3_revenue_target": 1800000.00,
                    "monthly_overhead_baseline": 15000.00,
                    "base_production_cogs_pct": (100.0 - gross_margin_baseline) / 100.0,
                    "y1_monthly_revenue_curve": [100000.00] * 12,
                    "historical_cash_flow_vector": [opening_cash + (2000.00 * i) for i in range(60)],
                    "historical_fa_nbv_vector": [opening_fa] * 5,
                    "historical_debt_vector": [0.0] * 5,
                    "historical_ar_vector": [opening_ar] * 5,
                    "historical_inventory_vector": [0.0] * 5,
                    "meta_project_name": project_title, "meta_horizon_years": forecast_extent_years, "meta_year_end": financial_year_end_month
                }
