# pages/app.py
# STRATA SUITE PRODUCTION ENGINE // DATA ENTRY DESK v6.9.1-PRODUCTION
...
st.title("🏛️ STRATA // Data Entry Panel")
st.markdown(
    f"🏭 **Active Sector Scope Constraints:** `{active_sic['sic_code']}` | Employer Burden: `{active_sic['base_er_nic_rate']*100}%` ER NIC"
)
st.caption(
    f"Active Scenario: `{st.session_state.get('active_project_name', 'Unsaved_Draft_Scenario')}`"
)
st.page_link("pages/onboarding.py", label="🕸️ Adjust Data Input Parameters Framework")
st.page_link("pages/reports.py", label="📊 Go directly to Performance Tab")
st.markdown("---")
...
# =========================================================================
# OPERATIONAL PARAMETER INPUT DESKS
# =========================================================================
with st.expander("📈 1. THE SALES DRIVER DESK", expanded=True):
    with st.form("sales_form", clear_on_submit=True):
        n = st.text_input(
            ...
            help="Provide a clear, descriptive name for this specific sales revenue stream.",
        )
        y1 = st.number_input(
        ...
        curve = st.selectbox(
            ...
            help="Select a standard profile or a custom saved seasonality shape curve curve defined inside Data Input Parameters.",
        )
        delay = st.selectbox(
...
with st.expander("💼 2. THE MILESTONE CONTRACT DESK"):
...
with st.expander("📦 3. THE PRODUCTION COGS DESK"):
...
with st.expander("💸 4. THE GENERAL OVERHEAD CARD", expanded=True):
...
with st.expander("🚜 5. THE FINANCED ASSET WIZARD"):
    with st.form("financed_form", clear_on_submit=True):
        ...
        # 🚀 UX CORRECTION: Asset-level depreciation rate entry integration
        asset_depr_rate = (
            st.number_input(
...
with st.expander("🏢 6. THE OUTRIGHT CAPEX CARD"):
    with st.form("outright_form", clear_on_submit=True):
        ...
        # 🚀 UX CORRECTION: Asset-specific depreciation rate
        asset_depr_rate = (
            st.number_input(
                "Asset-Specific Annual Depreciation Rate (% Straight-Line):",
                min_value=0.0,
...
with st.expander("👥 7. THE PERSONNEL HORIZON DESK"):
    with st.form("payroll_form", clear_on_submit=True):
        ...
        flex = st.slider("Annual Macro Wage Inflation Indexation %:", -10, 30, 0)
...
with st.expander("💰 8. THE FUNDING & EQUITY CARD"):
...
if st.sidebar.button(
    "🚪 Log Off Session", use_container_width=True, key="sb_logoff_app"
):
